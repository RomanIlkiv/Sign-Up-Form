...
<html lang="en">
    ...
    <body>
		...
		<section>
			<div id="news" class="col-lg-10 col-md-10 col-sm-10 col-xs-12 col-lg-offset-1 col-md-offset-1 col-sm-offset-1">
				<div id="news_menu">
					<a href="index.php" class="category">All categoies</a>
					<a href="index.php?category=sport" class="category">About Sport</a>	
					<a href="index.php?category=travel" class="category">About Travel</a>	
					<a href="index.php?category=science" class="category">About Science</a>	
				</div>
				<div id="articles">
					<? 
						if (!$_GET["category"]) {
							$param1 = "index.php?page=";
							$param2 = $db->query("SELECT `id`, `category`, `title`, `time` FROM `test_articles`");
							if ($_GET["page"] == "") {
								$_GET["page"] = 1;
								$result = $db->query("SELECT `id`, `category`, `title`, `time` FROM `test_articles` ORDER BY `time` DESC LIMIT 0, 3");
							}
							else {
								$start = ($_GET["page"] - 1) * 3;
								$result = $db->query(str_replace("START", $start, "SELECT `id`, `category`, `title`, `time` FROM `test_articles` ORDER BY `time` DESC LIMIT START, 3"));
							}
						}
						else if ($_GET["category"]) {
							$param1 = 'index.php?category='.$_GET["category"].'&page=';
							$param2 = $db->query("SELECT `id`, `category`, `title`, `time` FROM `test_articles` WHERE `category` = '".$_GET["category"]."'");
							if ($_GET["page"] == "") {
								$_GET["page"] = 1;
								$result = $db->query("SELECT `id`, `category`, `title`, `time` FROM `test_articles` WHERE `category` = '".$_GET["category"]."' ORDER BY `time` DESC LIMIT 0, 3");
							}
							else {
								$start = ($_GET["page"] - 1) * 3;
								$result = $db->query(str_replace("START", $start, "SELECT `id`, `category`, `title`, `time` FROM `test_articles` WHERE `category` = '".$_GET["category"]."' ORDER BY `time` DESC LIMIT START, 3"));
							}
						}
						...
						<div id="pagination" class="col-lg-6 col-md-6 col-sm-8 col-xs-12 col-sm-offset-2 col-lg-offset-3 col-md-offset-3 ">
						...
						if (isset($_GET["page"])) {
							$page = $_GET["page"];
							if ($page < 1) $page = 1;
							else if ($page > $count_pages) $page = $count_pages;
						}
						else {
							$page = 1;
						}
